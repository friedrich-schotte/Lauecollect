...
Server side of the Channel Access protocol, version 4.11.
Friedrich Schotte, 31 Oct 2009 -  Jul 7, 2017
Valentyn Stadnytskyi, Oct 27, 2018 - compatability with python 3.7
...
__version__ = "2.0.0" # almost competable with python 3.7 it doesn't 
...
def notify_subscribers_if_changed(PV_name,new_value):
    ...
    if not PV_name in PVs.keys(): return
    PV = PVs[PV_name]
    if new_value is None: delete_PV(PV_name); return
...
def notify_subscribers(PV_name):
    ...
    if value is  None: delete_PV(PV_name); return
    for address in PV.subscribers.keys():
    ...
    if command == "SEARCH":
        ...
        for PV_name in PVs.keys():
            ...
            if PV.channel_SID == channel_SID:
                ...
                reply = message("READ_NOTIFY",0,data_type,data_count,channel_SID,
...
def value(data_type,data_count,payload):
    ...
    if data_type.endswith("STRING"):
        ...
        if data_count > len(payload)/2: data_count = max(len(payload)/2,1)
        payload = payload.ljust(2*data_count,"\0")
        value = list(unpack(">%db"%data_count,payload[0:2*data_count]))
        if len(value) == 1: value = value[0]
...
if __name__ == "__main__": # for testing
    ...
    import threading # for enumerate()   
    from sim_motor import sim_motor,sim_EPICS_motor
    ##MirrorV = sim_EPICS_motor("14IDA:DAC1_4",name="MirrorV")
    ##print("MirrorV.EPICS_enabled = %r" % MirrorV.EPICS_enabled)
    MirrorV = sim_motor(name="MirrorV")
    print("register_object(MirrorV,'14IDA:DAC1_4')")
    print("unregister_object(MirrorV,'14IDA:DAC1_4')")
    print("PV_value('14IDA:DAC1_4.VAL')")
    print('start_server()')
    print('threading.enumerate()')
    print('import CAServer3')
    print('CAServer3.casput("test1","111")')
