...
__credits__ = []
__license__ = "GPLv3+"
__version__ = "3.0.9" # broadcast_addresses for Windows
    ...
    for name in interfaces:
        for info in interfaces[name]:
            ...
            if info.family == 2: # family 2 = IPv4
                if info.broadcast is not None: address = info.broadcast
                elif info.address is not None and info.netmask is not None:
                    address = broadcast_address(info.address,info.netmask)
                elif info.address is not None: address = info.address
...
def broadcast_address(address,netmask):
    ...
    broadcast_address_bits = unsigned_int(broadcast_address_bits)
...
def unsigned_int(value):
  ...
  if type(value) in [int,long] and value < 0: value = value+0x100000000
  return value
...
if __name__ == "__main__": # for testing
    ...
    PV_name = 'NIH:XRAY_SCOPE.TRACE_SOURCES'
