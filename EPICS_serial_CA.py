# This is a replacement for the Python serial module based on EPICS

# Friedrich Schotte, NIH 4 Oct 2008 - 4 Apr 2013
...
# EpicsCA is a EPICS to Python interface by Matt Newville, U Chigago,
...
class Serial(object):
    ...
    def write(self,string):
        """Send data"""
        caput(self.port+".TMOT",self.timeout,wait=True)
    ...
    def query(self,string,terminator="",count=0):
        ...
        t0 = time()
        current_timeout = caget(self.port+".TMOT")
        if time() - t0 > self.timeout: return ""
        if current_timeout != self.timeout:
            caput(self.port+".TMOT",self.timeout,wait=True,timeout=self.timeout)
        if time() - t0 > self.timeout: return ""
...
if __name__ == "__main__": # for testing - remove when done.
    port = Serial("14IDB-NIH:serial1")
    print r'port.query("SET:TEMP?\n")'
    print r'port.query("*IDN?\n")'
