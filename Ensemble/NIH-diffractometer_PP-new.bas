...
SETPARM 1023:UserString0,"NIH-diffractometer_PP.ab" 
...
LINEAR 5:LZi @20 
 PumpA_pos=PFBK(4)
LINEAR 4:200*CEIL((PumpA_pos-0.01)/200) @100 'Move PumpA to the next largest multiple of 200 before terminating program.	
HOME 4
PumpA_pos=0
...
PP_index=IGLOBAL(2)
END IF
DOUT:0::0, 1'mark time of START command
PVT 2: Zi, 0,4: PumpA_pos, 0,5: LZi, 0 @T_return |DOUT 0:0, 0 
...
T_error=(PCMD(2)-Z_mid)/Vmax-0.5*T_offset
DOUT:0::0, 1'mark time of PCMD(Z) command
DGLOBAL(0)=T_error
...
IF N_mode<5 THEN'Flythru-4, -12, -24, -48, -96
 Ti=12*DT
Zi=Zi+0.5*DZ
PVT 2: Zi, ZV,4: PumpA_pos, 0,5: LZi, 0 @Ti 
Ti=45*Nx*DT
Zi=Zi+45*DZ
PVT 2: Zi, ZV,4: PumpA_pos, 0,5: LZi, 0 @Ti 
Zi=Zi+0.5*DZ
...
IF IGLOBAL(1)=1 THEN'Trigger Digital Oscilloscope at appropriate time
...
'wait for next low-to-high transition
 i=0
Volt1=AIN:0::( 1)
DOUT:0::0, 2'marks time when loop starts
...
DOUT:0::0, 1'marks rising edge of 2-ms duration trigger pulse
 HALT
