...
'
' Version 1.1.7: reconcile all axes, except X and Y, at beginning and end
'
...
SETPARM 1023:UserString0,"PVT_Fly-thru.ab" 
...
scale_factor_array(1)=0.99999020000000005'APS 2017.02.26; 0.99999084 APS 2016.11.08; 0.99999525 'APS 03/07/2016
...
LINEAR 2:Zi @10 
LINEAR 5:LZi @10 
LINEAR 6:msShut_close1 @1000 
 PumpA_pos=PCMD(4)
LINEAR 4:50*CEIL((PumpA_pos-0.01)/50) @40 'Move PumpA to the next largest multiple of 50 before terminating program.	
HOME 4
PumpA_pos=PCMD(4)
...
CALL Synch()'Read msShut_Enable, PumpA_Enable, N_mode, and N_delay	
 IF IGLOBAL(0)=-1THEN GOTO cleanup END IF
...
END IF
PVT 2: Zi, 0,4: PumpA_pos, 0,5: LZi, 0,6: msShut_pos, 0 @Ti 
status=AXISSTATUS(4)' for debugging
ELSE
...
'Decode mode parameters from trigger pulse train
 ...
 SYNC
IF IGLOBAL(0)=-1THEN GOTO end_sync END IF
WEND
...
' Hard coded for flythru-4 (mode 10) or flythru-48 (mode 11)
'N_mode = 11
 END FUNCTION
...
T_ref=Ti'Last time used to compute scale_factor
 ...
 Z_error_max=0.20000000000000001
Z_error_min=-0.20000000000000001
Z_error_range=0.40000000000000002
T_corr_sum=0
