...
Z_0=12.779999999999999'13.19 'starting Z position
...
HOME 2
 END IF
MOVEABS 2:( Z_0-40.5*DZ) 'Got to starting position
WAIT MOVEDONE 2
...
PVT_INIT  @1 
PVT 2:(Z_0-40.5*DZ),0 @0.001 
j=0
WHILE 1
PumpA_pos=j*45
T_0=j*57*DT+DGLOBAL(0)+0.0040000000000000001' 4.33 Hz
FOR i=0 TO Nslots
Zi=Z_0-i*DZ
Ti=T_0+i*DT
IF i=0
...
PVT 2:(Zi+DZ/2),0 @Ti |DOUT 0:1, 1,1 
 ELSEIF i=Nslots
THEN
'PVT Z (Zi-DZ/2), 0 PumpA PumpA_pos, 0 TIME Ti DOUT X,1,0,1
PVT 2:(Zi-DZ/2),0 @Ti |DOUT 0:1, 0,1 
 IF j=0 THEN DGLOBAL(1)=0'reset PLL counter
 END IF
ELSE
