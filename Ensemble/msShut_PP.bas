...
'
HEADER
INCLUDE "msShut_parameters.basi"
END HEADER
...
STARTSYNC 1'set SYNCH interval to 1 ms
 Volt0=AIN:6::( 0)'capture voltage before starting inner loop 
 Volt1=Volt0
DOUT:6::0, 4'marks time of start
WHILE(Volt1<0.55000000000000004)OR(Volt1>2.8500000000000001)
SYNC
Volt1=AIN:6::( 0)'capture voltage within loop
 N_loop=N_loop+1
WEND
DOUT:6::0, 3'marks time of trigger pulse
HALT
...
SYNC'2ms
SYNC'3ms
Volt2=AIN:6::( 0)'capture voltage 3 ms after trigger 
DOUT:6::0, 2'marks time when volt2 is captured

'Determine mode and parameters for PVT trajectory
 IF(Volt0<1.6000000000000001)AND(Volt2<1.6000000000000001)THEN'trigger mode rising edge
...
Ti=2*(12*DT-(7+T_offset)/1000)'seconds
...
Vmax=slope*1.5*(2*msShut_step)/(Ti*1000)'deg/ms
 T_delta=11+T_offset-12000*DT'milliseconds
'the number 11.0 corresponds to the number of SYNC commands (5+6=11)
 N_delay=6
SYNC'4ms
SYNC'5ms
ELSEIF(Volt0<1.6000000000000001)AND(Volt2>1.6000000000000001)THEN'gating mode rising edge
...
-T_shift'adding 0.25 starts motion 0.25 ms earlier
 msShut_pos=msShut_open+slope*msShut_step
midpoint=msShut_open+0.5*slope*msShut_step
Ti=2*(13*DT-(9+T_offset)/1000)'seconds
...
Vmax=slope*1.5*msShut_step/( Ti*1000)'deg/ms
 T_delta=12+T_offset-13000*DT'milliseconds
...
'PVT move
PVT 6: msShut_pos, 0 @Ti |DOUT 6:0, 0 'mark PVT starting time
 START'Start occurs after 2 ms dead time.
DOUT:6::0, 1'mark time of START command
...
DOUT:6::0, 1'marks time at trajectory mid-point
 Pos_error=PCMD(6)-midpoint-Vmax*T_delta
