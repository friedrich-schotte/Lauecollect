"""
Monitors the timing stability of the seed laser of the picosecond
laser system.
It measures the phase of the Tsunami output pulse train using the
Laser diagnostics oscilloscope.

Setup: On the laser diagnostics oscilloscope, define the Measurement 2 as
...
nom_delay = 1.24e-9 # calibration constant (range 0 to 14.2 ns)
default_logfile = "//id14bxf/data/Lok-to-Clock Stability.log"
naverage = 10
...
def delay():
    ...
    losct_dial = floor(losct.count/4)*4 * losct.stepsize
    t = delay_measurement.value - psd1.dial + losct_dial
    t = (t - nom_delay + period/2) % period - period/2 + nom_delay
    return t
