...
class PolyPoints:
    ...
    def __init__(self, points, attr):
        self._points = numpy.array(points).astype(numpy.float64)
...
class PlotCanvas(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        # Problems with zlib decompression, Friedrich 25 Feb 2008
        try:
            self.HandCursor = wx.Cursor(getHandImage())
        except:
            self.HandCursor = wx.Cursor(wx.CURSOR_HAND)
        try:
            self.GrabHandCursor = wx.Cursor(getGrabHandImage())
        except:
            self.GrabHandCursor = wx.Cursor(wx.CURSOR_HAND)
        try:
            self.MagCursor = wx.Cursor(getMagPlusImage())
        except:
...
def getMagPlusBitmap():
    return wx.Bitmap(getMagPlusImage())


def getMagPlusImage():
    stream = io.BytesIO(getMagPlusData())
...
def getGrabHandBitmap():
    return wx.Bitmap(getGrabHandImage())


def getGrabHandImage():
    stream = io.BytesIO(getGrabHandData())
    return wx.Image(stream)
...
def getHandBitmap():
    return wx.Bitmap(getHandImage())


def getHandImage():
    stream = io.BytesIO(getHandData())
    return wx.Image(stream)
