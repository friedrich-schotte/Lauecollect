...
class XRay_Beam_Center_Parameters_Panel(BasePanel):
    ...
    @monitored_property
    def title(self):
        return f"X-Ray Beam Center Par. [{self.domain_name}]"

    @property
    def name(self):
        return f"XRay_Beam_Center_Parameters_Panel.{self.domain_name}"
    ...
    def parameters(self):
        return [
            [("X0", self.instrument, "nominal_beam_center_x", "int"), {"unit": "pixels"}],
            [("Y0", self.instrument, "nominal_beam_center_y", "int"), {"unit": "pixels"}],
            [("Imin", self.instrument, "I_min", "float"), {"unit": "counts"}],
        ]
...
if __name__ == '__main__':
    ...
    redirect(f"{domain_name}.{XRay_Beam_Center_Parameters_Panel.__name__}", format=msg_format)

    app = wx.GetApp() if wx.GetApp() else wx.App()
    self = XRay_Beam_Center_Parameters_Panel(domain_name)
    app.MainLoop()
