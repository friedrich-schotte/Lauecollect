...
"""
__version__ = "1.16" # finish_series
...
class Collect(object):
    ...
    def set_finish_series(self,value):
        self.last_image = self.n_collect(self.current)-1

    def finish_collection_at_pass(self,collection_pass):
        finish = False
        if collection_pass in self.collection_passes:
            for (first,last) in self.collection_pass_ranges(collection_pass):
                if self.finish_collection_at_image(first): finish = True
        if finish: info("Finish collection at pass %r? Yes" % (collection_pass+1))
        return finish
    
    def finish_collection_at_image(self,image_number):
        finish = image_number >= self.n_collect(image_number-1)
        if finish: info("Finish collection at image %r? Yes" % (image_number+1))
        return finish

    def n_collect(self,image_number):
        n_collect = self.n
        if self.finish_series:
            period = 1
            for i,variable in enumerate(self.collection_variables):
                period *= len(self.variable_values(i))
                if variable == self.finish_series_variable: break
            n_collect = (int(image_number) / period + 1) * period
        return n_collect
    ...
    def collect_dataset(self):
        ...
        for collection_pass in self.collection_passes:
            if self.finish_collection_at_pass(collection_pass): break
            if self.cancelled: break
            for (first,last) in self.collection_pass_ranges(collection_pass):
                ...
                for i in range(first,last+1):
                    if self.finish_collection_at_image(i): break
                    if self.cancelled: break
                    self.acquire(i)
    ...
    def timing_system_set_last(self,last):
        self.actual("Timing system set last image %r" % (last+1))
        N = self.xray_images_per_sequence_queue
        from timing_sequence import timing_sequencer
        timing_sequencer.queue_max_repeat_count = (last+1)/N
...
if __name__ == '__main__':
    ...
    ##print('')
    print('self.finish_series = %r' % self.finish_series)
    print('self.finish_series_variable = %r' % self.finish_series_variable)
    print('self.finish_collection_at_image(595)')
    print('self.finish_collection_at_pass(0)')
    print('self.n_collect(0*595)')
