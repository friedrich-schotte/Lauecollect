...
timing_system = timing_system(timing_system_name)
Sequence = timing_system.composer.Sequence
Sequences = timing_system.composer.Sequences


def safe_expand_sequence(expr):
    values = []
    if expr:
        try:
            expr = expand_sequence(expr)
        except Exception as msg:
            logging.error("expand_sequence: %r: %s\n%s" % (expr, msg, traceback.format_exc()))
            expr = ""
    if expr:
...
def name(operation): return capitalize.capitalize()
...
if __name__ == "__main__":
    ...
    # s = "(nan, PLP=Period-48, SEQ=010)*5, (nan, PLP=Period-144, SEQ=100), (264+1*144, SEQ=101), (nan, SEQ=100)*2, (264+4*144, SEQ=101), (nan, SEQ=100)*4, " \
    #    "(264+9*144, SEQ=101), (nan, SEQ=100)*8, (264+18*144, SEQ=101), (nan, SEQ=100)*16, (264+35*144, SEQ=101), (nan, SEQ=100)*32, (264+68*144, SEQ=101)"
    # s = "(-10us, PLP=Flythru-4), -10us, (264, SEQ=1010), 528, 792, 1056, (-10us, SEQ=1111), -10us"
    # s = '[enable=011]*4+[enable=111]'
    # s = "[(pp=Period-48, enable=010)]*5, (image=0, pp=Period-144, enable=100), (264+1*144, enable=101), [(image=0, enable=100)]*2, (264+4*144, enable=101), "\
    #    "(image=0, enable=100)*4, (264+9*144, enable=101), (image=0, enable=100)*8, (264+18*144, enable=101), (image=0, enable=100)*16, (264+35*144, " \
    #    "enable=101), (image=0, enable=100)*32, (264+68*144, enable=101)"
    # s = '{enable:111}'
    # s = 'delays=[off,100ps,off,1ns]'
    s = "-16, 12.2, 65.2, 75.2, 85.2, 95.2, 105.2, 119.76"
    ...
    # print("x = expand_sequence(sequence_modes.acquisition.value,report=logging.debug)")
    # print("x = expand_sequence(scan_configuration.points.value,report=logging.debug)")
    # print("x = expand_sequence(temperature_configuration.list.value,report=logging.debug)")
    # print("x = expand_sequence(power_configuration.list.value,report=logging.debug)")
    # print("x = delay_sequences(delay_configuration.delay_configuration.value,report=logging.debug)")
    # print('max(arange(-2,2,0.05))')
    # print(f'delay_sequences({s!r}, report=logging.debug)')
    print(f'x = expand_sequence({s!r}, report=logging.debug)')
