...
class monitored_property(property):
    ...
    def handle_input_change(self, instance, input_count, event):
        ...
        with self.cache(instance).lock:
            if self.event_valid(instance, event, input_count):
                input_references = self.cache(instance).input_references
                input_references[input_count].event_history.add(event)

                if self.inputs_values_valid(instance, event.time):
    ...
    def update_event_history(self, instance, time):
        if self.inputs_values_valid(instance, time):
            input_references = self.cache(instance).input_references
    ...
    def inputs_values_valid(self, instance, time):
        input_references = self.cache(instance).input_references
        valid = True
        for ref in input_references:
            if not ref.event_history.has_value(time):
                logging.debug(f"{self.repr(instance)}: inputs values are not valid because {ref.event_history} has no value before or at {date_time(time)}")
                valid = False
        return valid
    ...
    def event_valid(self, instance, event, input_count):
        from time import time as now
        valid = True

        input_references = self.cache(instance).input_references
        input_reference = input_references[input_count]
        if event and event.reference != input_reference:
            logger.warning(f"{self.repr(instance)}, input {input_count}: Asked for updates of {input_reference}, instead got update of {event.reference}")

        dt = event.time - now()
        # if abs(dt) > 10 and dt < 0:
        #     logging.debug(f"{self.repr(instance)}, input {input_count}: Ignoring {event} {abs(dt)} s in the past")
        #    valid = False
        if abs(dt) > 10 and dt > 0:
            logging.debug(f"{self.repr(instance)}, input {input_count}: Ignoring {event} {abs(dt)} s in the future")
            valid = False
        return valid
