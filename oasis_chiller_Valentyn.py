...
__version__ = "2.5.0" # Python 3 compatibility and improved init_communications
...
class OasisChillerDriver(object):
    ...
    def set_value(self,parameter_number,value):
        ...
        if len(reply) != 1:
            warning("expecting 1, got %d bytes" % len(reply)); return
        reply_code, = unpack('B',reply)
        if reply_code != code: warning("expecting 0x%X, got 0x%X" % (code,reply_code))
...
class OasisChiller_IOC(object):
    ...
    def donemove(self, val, rbv):
        """
        returns True is setT is within 0.3 from RBV T
        """
        return abs(val - rbv) < 0.3
    ...
    def update_once(self):
        ...
        if online:
            if online and not self.was_online:
                ...
                val = oasis_chiller_driver.VAL
                casput(self.prefix+".VAL",val)
                casput(self.prefix+".RBV",rbv)
                casput(self.prefix+".DMOV",self.donemove(val,rbv))
