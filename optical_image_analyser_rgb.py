...
class Image_analyser(Thread):
    ...
    pixels_to_use1 = persistent_property('pixels to use1', (300,700))
    pixels_to_use2 = persistent_property('pixels to use2', (300,700))
    ...
    def __init__(self, name = 'sample_frozen_optical' , pixels_to_use = (509,515)):
        ...
        if self.orientation == 'vertical':
            self.pixels_to_use_1 =  (512+20,712+20)
            self.pixels_to_use_2 =  (1020+20,1220+20)
    ...
    def convert_masked_array(self,arr,pixels_to_use):
        from numpy import zeros
        if self.orientation == 'vertical':
            arr_res = zeros((arr.shape[0]+1,pixels_to_use[1]-pixels_to_use[0]+1,arr.shape[2]))
    ...
    def is_new_image(self):
        ...
        c = time() - a
        if self.imageCounter - frame_count > 1000:
            self.imageCounter = 0
        d = time() - a
    ...
    def get_current_array(self):
        ...
        if self.is_new_image():
            self.current_array_lst = []
            self.current_array_lst.append(self.convert_masked_array(self.get_image(),self.pixels_to_use_1))
            self.current_array_lst.append(self.convert_masked_array(self.get_image(),self.pixels_to_use_2))
            res = True
        else:
            res = False
        return res
    ...
    def plot(self):
        plt.subplot(121)
        plt.imshow(self.difference_image-self.background_image)
        plt.colorbar()
        plt.show()
