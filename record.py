...
class Record(object):
        ...
        if name.startswith("__") and name.endswith("__"):
            ...
            if not self.__equal__(old_value,value):
                ...
                for (dep,old_value,new_value) in zip(deps,old_values,new_values):
                    ...
                    if changed: self.__notify__(dep)
        ...
        for dep in deps:
            for prop in dep_dict:
                if dep in dep_dict[prop]:
                    if not prop in deps: deps.append(prop)
        return deps
    ...
    def __dep_dict__(self):
        ...
        for name in self.__property_names__:
            ...
            if len(dep_dict[name]) == 0: del dep_dict[name]
        return dep_dict
    ...
    def __methods_names_to_invoke__(self,name):
        ...
        for methods_name in self.__method_names__:
            ...
            if name in property_names: names.append(methods_name)
        return names
    ...
    def __property_names__(self):
        ...
        names = dir(type(self))
        names = [name for name in names if not name in base_class_property_names]
    ...
    def __is_private__(self,name):
        return name.startswith("__") and name.endswith("__")
    
    def __equal__(self,current_value,value):
        return (repr(current_value) == repr(value))

    def monitor(self,name,function,*args,**kwargs):
        if not name in self.__callbacks__: self.__callbacks__[name] = []
...
def depends_on(*args):
    ...
    def decorate(method):
        ...
        for arg in args:
            if not arg in property_names: property_names.append(arg)
...
class callback(object):
    ...
    def __repr__(self):
        ...
        return "%s(%s)" % (self.function.__name__,arglist)
