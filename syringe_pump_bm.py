...
Brian Mahon, Friedrich Schotte, Philip Anfinrud, Naranbaatar Dashdorj,
...
def pup1():
    """When both pumps are connected, this runs injections a small volume from
    P1 towards P2, to increase pressure (by X atm) for diagnostic purposes, 
    and returns remaining volume of P1"""
    P1.port = 'In'
...
def pup2():
    """When both pumps are connected, this runs injections a small volume from
    P2 towards P1, to increase pressure (by X atm) for diagnostic purposes, 
    and returns remaining volume of P2"""
    P2.port = 'In'
    while(P1.moving or P2.moving): sleep(0.1)
    P2.speed = 0.1
    P2.volume -= 0.3
...
def pdown1():
    """When both pumps are connected, this reverse injects a small volume from
    P1 away from P2, to decrease pressure (by X atm) for diagnostic purposes, 
    and returns remaining volume of P1"""
    P1.port = 'In'
    while(P1.moving or P2.moving): sleep(0.1)
    P1.speed = 0.1
...
def pdown2():
    """When both pumps are connected, this reverse injects a small volume from
    P2 away from P1, to decrease pressure (by X atm) for diagnostic purposes, 
    and returns remaining volume of P2"""
    P2.port = 'In'
    while(P1.moving or P2.moving): sleep(0.1)
    P2.speed = 0.1
...
def purge():
    """This sequence will load P2 syringe with oil as it is attached to 
    position 1 and dispense throughout puck and 1 m capillary to purge 
    system prior to use. Sequence will end with P2 filled with oil. 
    Note P2 will empty prior to filling"""
    P2.port = 'Out'
...
def fill(): 
    """This sequence will load P2 syringe with oil """
    P2.port = 'Out'
    while (P2.moving): sleep(0.1)
    P2.speed = 10
    P2.volume = 0
...
def run(s1, s2):
    """This sequence loads 1 m long capilary with the intention of 
    generating alternating (1:5) droplets for crystal growth. 
    To run, speeds for P1 (s1) and P2 (s2) need to be defined). 
    Sequence will load 1 (approx. 50ul) capillary. After sequence P2 will 
    purge ith 5ul oil to ensure droplets fully loaded in cappilary and account 
    for dead volume"""
    P1.port = 'In'
    P2.port = 'In'
    while (P1.moving or P2.moving): sleep(0.1)
