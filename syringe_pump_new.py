...
__version__ = "5.8.1" # signs and offsets for P1,P1 
...
class SyringePumpCombined(object):
  ...
  def limit_V(self,V):
    """Apply a limit check to a new value for dV
    Return value: a value of dV that is within the allowed range"""
  ...
  def get_V_min(self):
    """in units of μL"""
    V_min_p1 = min([self.V_from_V1(V1) for V1 in self.p1.min,self.p1.max])
    V_min_p2 = min([self.V_from_V2(V2) for V2 in self.p2.min,self.p2.max])
    V_min = max(V_min_p1,V_min_p2)
    return V_min
  ...
  def get_V_max(self):
    """in units of μL"""
    V_max_p1 = max([self.V_from_V1(V1) for V1 in self.p1.min,self.p1.max])
    V_max_p2 = max([self.V_from_V2(V2) for V2 in self.p2.min,self.p2.max])
    V_max = min(V_max_p1,V_max_p2)
    return V_max
  ...
  def V_from_V1(self,V1):
    """To which value does V change when p1 moves to V1?"""
    # V1 = self.p1.command_value + dV
    dV = V1 - self.p1.command_value
    # dV = V - self.V
    V = dV + self.V
    return V

  def V_from_V2(self,V2):
    """To which value does V change when p2 moves to V2?"""
    # V2 = self.p2.command_value + dV
    dV = V2 - self.p2.command_value
    # dV = V - self.V
    V = dV + self.V
    return V
  ...
  def limit_dV(self,dV):
    """Apply a limit check to a new value for dV
    Return value: a value of dV that is within the allowed range"""
    from numpy import clip
    dV = clip(dV,self.dV_min,self.dV_max)
    return dV

  def get_dV_min(self):
    """in units of μL"""
    dV_min_p1 = min([self.dV_from_V1(V1) for V1 in self.p1.min,self.p1.max])
    dV_min_p2 = min([self.dV_from_V2(V2) for V2 in self.p2.min,self.p2.max])
    dV_min = max(dV_min_p1,dV_min_p2)
    return dV_min
  ...
  def get_dV_max(self):
    """in units of μL"""
    dV_max_p1 = max([self.dV_from_V1(V1) for V1 in self.p1.min,self.p1.max])
    dV_max_p2 = max([self.dV_from_V2(V2) for V2 in self.p2.min,self.p2.max])
    dV_max = min(dV_max_p1,dV_max_p2)
    return dV_max
  ...
  def dV_from_V1(self,V1):
    """To which value does dV change when p1 moves to V1?"""
    # V1 = self.p1.command_value - ddV/2
    # self.p1.command_value - ddV/2 = V1
    # -self.p1.command_value + ddV/2 = -V1
    # ddV/2 = self.p1.command_value - V1
    ddV = 2*(self.p1.command_value - V1)
    # ddV = dV - self.dV
    # dV - self.dV = ddV
    dV = ddV + self.dV
  ...
  def dV_from_V2(self,V2):
    """To which value does dV change when p2 moves to V2?"""
    # V2 = self.p2.command_value + ddV/2
    # self.p2.command_value + ddV/2 = V2
    # ddV/2 = -self.p2.command_value + V2
    # ddV = 2*(-self.p2.command_value + V2)
    ddV = -2*(self.p2.command_value - V2)
    # ddV = dV - self.dV
    # dV - self.dV = ddV
