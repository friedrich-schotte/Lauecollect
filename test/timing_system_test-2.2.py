...
"""
__version__ = "2.2"
from instrumentation import timing_system,lxd,bcf,psod,psod1,clksrc,lecroy_scope
from scan import rscan,timescan as tscan
from sleep import sleep
delay = lecroy_scope("pico21").measurement(1)
tmax = 1/bcf
npoints = tmax/psod.stepsize

def scan():
    lxd.value = 0
    for i in range(5,25):
        rscan([lxd,delay.gate.start,delay.gate.stop],0,[tmax,-tmax,-tmax],
            npoints,delay,averaging_time=2.1,
            logfile="logfiles/delay-%d.log"%i)

def timescan():
    tscan(delay,averaging_time=4.0,logfile="logfiles/delay.log")
