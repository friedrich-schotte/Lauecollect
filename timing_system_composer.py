...
@cached_function()
def timing_system_composer(name):
    return Timing_System_Composer(name)


class Timing_System_Composer(object):
    ...
    def __repr__(self):
        return "%r.composer" % self.timing_system
    ...
    def Sequences(self, delay=None, sequences=None, **kwargs):
        from timing_system_sequences import Sequences
        return Sequences(self.name, delay=delay, sequences=sequences, **kwargs)

    @property
    def db_name(self):
        return "timing_system/%s/composer" % self.name
    ...
    def command_value(self, name):
        value = self.timing_system.parameter(name, self.default_values[name])
    ...
    timing_system_sequence = alias_property("timing_system.sequence")

    @monitored_property
    def sequence(self, timing_system_sequence):
        return timing_system_sequence

    @sequence.setter
    def sequence(self, value):
        self.timing_system_sequence = value
        self.update_later = True

    acquisition_sequence = alias_property("timing_system.acquisition_sequence")
    ...
    def sequencer_packet_generate(self, sequence):
        ...
        Return value: binary data + descriptive string
        """
        from timing_system_sequencer import sequencer_packet
    ...
    def register_specs(self, sequence):
        ...
        T_base = self.timing_system.hsct
        n = sequence.period
        ...
        for i_channel in range(0, len(self.timing_system.channels)):
            ...
            if channel.PP_enabled:
                if channel.special == "trans":  # Sample translation trigger
                    ...
                    pso_period = 5 * self.timing_system.bct
                    ...
                    pso_dial = self.timing_system.psod3.dial_from_user(pst_dial) % pso_period
                    psod3_dial = floor(pso_dial / pso_coarse_step) * pso_coarse_step
                    psod3_count = self.timing_system.psod3.count_from_dial(psod3_dial)
                    ...
                    clk_shift_count = self.timing_system.psod2.count_from_dial(psod2_dial)
    ...
    def parameter_description(self):
        ...
        # description += "high_speed_chopper_phase.value=%.12f," % self.timing_system.high_speed_chopper_phase.value
        # description += "high_speed_chopper_phase.offset=%.12f," % self.timing_system.high_speed_chopper_phase.offset
        # description += "hsc.delay.offset=%.12f," % self.timing_system.channels.hsc.delay.offset
        description += "xd=%.12f," % self.timing_system.xd
        ...
        for i_channel in range(0, len(self.timing_system.channels)):
            ...
            if channel.PP_enabled:
                if channel.special == "pso":
                    description += "psod3.offset=%.12f," % self.timing_system.registers.psod3.offset
        ...
        description += "generator=%r," % "composer"
        description += "generator_version=%r," % __generator_version__
        import timing_system_sequencer
        description += "timing_sequence_version=%r," % timing_system_sequencer.__generator_version__
    ...
    def get_xd(self):
        return self.timing_system.xd

    def set_xd(self, value):
        if self.timing_system.xd != value:
            self.timing_system.xd = value
            self.update_later = True
    ...
    tick_period = alias_property("timing_system.hsct")
...
if __name__ == "__main__":
    ...
    self = timing_system_composer(name)
    print('')
    ...
    print('')
    print('self.sequencer.cache_size = %r' % self.sequencer.cache_size)
    print('self.sequencer.remote_cache_size = %r' % self.sequencer.remote_cache_size)
    print('')
    ...
    channel = self.timing_system.channel(i_channel)
