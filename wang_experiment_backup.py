...
timepoints = [1,2,5,10]
nlaser = 1
directory = "/net/mx340hs/data/wang_0815/Test/Test1"
file_basename = "Test1"

filenames = ["%s/%s_%gs.mccd" % (directory,file_basename,t) for t in timepoints]

dt = timing_system.hsct*48
it0 = max(int(ceil(nlaser/2.)),2) # number seqeunces before t=0
N = it0 + int(rint(max(timepoints)/dt))+1+20
# Laser pulse burst is centered at t=0.
laser_on = array([0]*N)
nlaser1 = nlaser/2; nlaser2 = nlaser-nlaser1
