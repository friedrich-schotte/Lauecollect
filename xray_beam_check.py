...
while recorded the transmitted pulse energy through with the X-ray oscilloscope
...
from instrumentation import MirrorH, MirrorV, shg, svg
from instrumentation import xray_pulse
from logfile import LogFile
from persistent_property import persistent_property


class Xray_Beam_Check:
    ...
    class Settings:
        ...
        def x_aperture_control(self):
            """Which motor to use to narrow down the horizontal aperture"""
            return shg
        ...
        # To narrow down aperture upstream of the detector for higher sensitivity
    ...
    def x_control_corrected(self):
        if len(self.x_scan_x) < 3 or len(self.x_scan_I) < 3:
            return nan
    ...
    def y_control_corrected(self):
        if len(self.y_scan_y) < 3 or len(self.y_scan_I) < 3:
            return nan
    ...
    def I_sigI(self):
        ...
        sI = xray_pulse.stdev
        N = xray_pulse.count
        sigI = sI / sqrt(N - 1) if N > 1 else nan
        return I, sigI


xray_beam_check = Xray_Beam_Check("BioCARS")
